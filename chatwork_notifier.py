...
# -*- coding: utf-8 -*-
"""
chatwork_notifier.py
===========================================================================
手動アプローチBotの実行結果・重大エラーをChatworkへ通知するモジュール。

Chatwork REST API（POST /rooms/{room_id}/messages）を使う。
ブラウザ自動操作（Selenium）は使わない
  - Chatworkはログイン時にreCAPTCHA/端末認証が入ることがあり、
    自動ログインが恒久的に詰まるリスクがあるため。
  - API Token は Chatwork の全プラン（無料プラン含む）で
    「設定 → API連携」から誰でも取得できる標準機能であり、
    追加契約は不要。

───────────────────────────────────────────────────────────────────────
設定ファイル: chatwork_config.json
───────────────────────────────────────────────────────────────────────
exe/.py と同じフォルダに配置する。中身は chatwork_config.json.example を
コピーして作成し、api_token を実際の値に書き換えること。

  {
    "api_token": "ここに取得したAPIトークンを貼り付け",
    "room_id": "443611594",
    "boss_account_id": "10357331",
    "boss_name": "kobayashi",
    "test_mode": false
  }

  - room_id        : 通知を送るグループチャットのID
                      （URL の #!rid の後ろの数字）
  - boss_account_id: メンション対象者のアカウントID
                      （room_members_lookup.py で調べられる）
  - boss_name      : メンション表示名（Chatwork上の名前と一致させる
                      必要はない。[To:id]の直後に表示される文字列）
  - test_mode      : true にすると、
                        (a) 送信するメッセージの先頭に必ず
                            "🧪【テスト通知】" が付く
                        (b) 上司へのメンション（[To:id]）を付けない
                      ため、誤って本番のroom_idのまま実行しても
                      上司に通知が飛ばない・メンションされない。
                      動作確認が終わったら false に戻すこと（省略時は
                      false 扱い）。

このファイルが存在しない、または必須項目が空の場合、通知機能は自動的に
無効化される（＝bot本体の動作には一切影響しない。ログにWARNを出すのみ）。
===========================================================================
"""
